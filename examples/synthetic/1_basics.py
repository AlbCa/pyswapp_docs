...
swam = BaseManager(f'{prj_dir}/proc/0_basics', # project directory path
...
# Read/Write FK filter
# 1. select data
swam.select_data(sin=1, rep=1, inplace=True)
swam.plot('FK')

# 2. read filter for one data
# 2.a read filter from file (if exists)
points_top, points_bot = utils.read_filter('/home/Natalie/Downloads/FKfilter.txt')

print(points_top)

# # 2.b read filter from database (if exists)
# points_top, points_bot = swam.read_filter()

# # 3. apply filter to data
# swam.apply_filter(ftype='FK', points_top=points_top, points_bot = points_bot)
# swam.plot('FK')
#
# # 4. write filter to file (key defines whether its a top or bottom filter, 't' or 'b', respectively)
# utils.write_filter('FKfilter_v2.txt', points_top, key='t')