...
procset = 'proc4' # processing set label
...
# transform data for all files (attr='fdbf', apply_to = 'all', use_windows=True)
swam.transform(attr='phaseshift', apply_to = 'all', use_windows=True)
# swam.transform(attr='fdbf', apply_to = 'all', use_windows=True)

# manual dc picking
swam.gui_interact('FV',use_windows=True)

# extract the dispersion curves automatically
swam.extract_curves(apply_to = 'all', method = 'max', use_windows=True)
