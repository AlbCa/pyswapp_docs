...
class Curve:
    ...
    def __init__(self,mode = 0, wave = 'rayleigh', type = 'phase'):
        ...
        if type not in ['phase', 'group', 'ellipticity']:
            raise ValueError('Data type must be "group" or "phase" velocity or ellipticity.')

        self._mode = mode
        self._wave = wave
        self._type = type

    def _init_data(self,freq, vel, err = None):
        """initialize data"""
    ...
    def _read(self, fname):
        ...
        self._init_data(dat[:, 0], dat[:, 1], err)
    ...
    def _set(self,param, value, orig = True):
        """set a parameter"""
    ...
    def _resample(self, pmin, pmax, pn, pspace = 'log', param = 'f', kind = 'cubic', inplace = False):
        """resample data"""
        ...
        if param in ['f','lam']:
            ...
            if param == 'lam':
                parx_new = self._frequency(parx_new, pary_new)

            if inplace:
                self._init_data(freq=parx_new,vel=pary_new,err=err_new)
                self.npts = len(self.data)
            else:
                curve_new = Curve(self._mode,self._wave,self._type)
                curve_new._init_data(freq=parx_new,vel=pary_new,err=err_new)
    ...
    def _smooth(self,kernel_size=5,inplace = False):
        """smooth data"""
    ...
    def _markInvalid_by_poly(self, vertices):
    ...
    def _markInvalid(self, pmin=None, pmax=None, param = 'f'):
    ...
    def _dropInvalid(self,inplace):
        ...
        if inplace:
            self._init_data(freq=data['f'], vel=data['vr'], err=data['err'])
            self.npts = len(self.data)
        else:
            curve_new = Curve(self._mode, self._wave, self._type)
            curve_new._init_data(freq=data['f'], vel=data['vr'], err=data['err'])
            return curve_new

    def _save(self, prjdir, pre, format, parkseis_params=None):
    ...
    def _plotdata(self, data = None, axes=None, outfile=None, fmt=None, show=True, show_orig = False, **kwargs):
    ...
    def _plotColumn(self, data = None, axes=None, outfile=None, fmt=None, show=True, **kwargs):
