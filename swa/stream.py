...
class SeismicStream:
    ...
    def fp2fv(self, f, p, R_fp):
        """
        Frequency–slowness to frequency–velocity transform
        ...
        # Sort velocity axis (important!)
        order = np.argsort(v)
        v = v[order]
        R_fp = R_fp[order, :]

        # Finest velocity sampling
        dv = np.min(np.abs(np.diff(v)))

        # New interpolation grid
        f_int = f
        v_int = np.arange(v.min(), v.max() + dv, dv)

        F_int, V_int = np.meshgrid(f_int, v_int)

        # Build interpolator (original grid is (v, f))
        ...
        # Query points
        pts = np.column_stack((V_int.ravel(), F_int.ravel()))
        R_fv = interp_fun(pts).reshape(V_int.shape)

        freq = F_int[0,:]
        vel = V_int[:,1]

        return freq, vel, R_fv

    def _inverse_radon(self,
                                 weights=None,
                                 line_model="linear",
                                 inversion_model="L2",
        ...
        # Load amplitudes
        st = self._pst.copy() if self._pst is not None else self._st.copy()
        M = self._amps(st=st)  # shape: (n_offsets, n_time)

        if self.pad:
            M, _, _ = self._zero_padding(M)
        if self.norm_amps:
            M = self._normalize_amps(M)

        nX, nT = M.shape
        nF = int(2 ** np.ceil(np.log2(nT)))  # FFT size
        ...
        # Offset reference
        ref_dist = np.mean(offsets)
        Dist = offsets

        # ---------------------------------------------------------
        # Frequency axis (correct definition)
        # ---------------------------------------------------------
        freq = np.fft.fftfreq(nF, d=dt)
        nf = nF // 2  # positive frequencies
        freq_pos = freq[:nf]

        # Frequency band of interest
        fid_min = np.searchsorted(freq_pos, self.fmin)
        fid_max = np.searchsorted(freq_pos, self.fmax)

        # ---------------------------------------------------------
        # Phase velocity / slowness axis
        # ---------------------------------------------------------
        vels = np.arange(self.vmin, self.vmax, self.vstep)
        p = 1.0 / vels  # slowness
        np_slowness = len(p)
        ...
        # Preprocessing
        # ---------------------------------------------------------
        # Apply cosine taper to each trace (time domain)
        for i in range(nX):
            M[i, :] = self._cos_taper(M[i, :])

        # FFT in time
        Mfft = np.fft.fft(M, n=nF, axis=1)

        # Weights
        if weights is None:
            weights = np.ones(nX)
        W = sparse.spdiags(weights, 0, nX, nX)

        # ---------------------------------------------------------
        # Build Radon time-shift matrix
        # ---------------------------------------------------------
        Tshift = np.zeros((nX, np_slowness))

        for k in range(np_slowness):
            if line_model.lower() == "parabolic":
                Tshift[:, k] = (2 * ref_dist * p[k] * Dist +
                                p[k] * Dist ** 2)
            else:
                Tshift[:, k] = p[k] * Dist

        # Identity for regularization
        I = sparse.eye(np_slowness)
        ...
        # Allocate Radon spectrum
        Rfft = np.zeros((np_slowness, nf), dtype=complex)

        # Frequency loop
        for i in range(fid_min, fid_max):

            f = freq_pos[i]

            # Radon forward operator
            A = np.exp(2j * np.pi * f * Tshift)

            AtA = A.T.conj() @ (W @ A)
            AtM = A.T.conj() @ (W @ Mfft[:, i])

            mu = hyperparameters[0] * np.abs(np.trace(AtA))

            # L2 solution
            Rcur = np.linalg.solve(AtA + mu * I.toarray(), AtM)
            ...
            if inversion_model.upper() in ["L1", "CAUCHY"]:
                ...
                for _ in range(20):
                    if inversion_model.upper() == "CAUCHY":
                        Q = sparse.spdiags(
                            1.0 / (np.abs(Rcur) ** 2 + b), 0, np_slowness, np_slowness
                        )
                    else:
                        Q = sparse.spdiags(
                            1.0 / (np.abs(Rcur) + b), 0, np_slowness, np_slowness
                        )

                    Rnew = np.linalg.solve(AtA + lam * Q.toarray(), AtM)

                    if np.linalg.norm(Rnew - Rcur) / np.linalg.norm(Rcur) < 1e-3:
                        break
        ...
        Rfft = np.abs(Rfft)  # amplitude
        Rfft /= Rfft.max(axis=0, keepdims=True) + 1e-12

        freq, vel, R_fv = self.fp2fv(freq_pos[fid_min:fid_max], p, Rfft[:,fid_min:fid_max])

        fig, ax = plt.subplots(figsize=(6, 8))

        im = ax.imshow(
            R_fv,
            extent=[freq_pos[0], freq_pos[-1], vels[-1], vels[0]],
            aspect="auto",
        ...
        # F, C = np.meshgrid(freq_pos, vels)
        # K = 2 * np.pi * F / C

        self.frequency = freq
        self.dispersive_energy = R_fv
        self.velocity = vel
        self.wavenumber = np.zeros_like(vel)
    ...
    #
    #     # frequency range
    #     freq = np.fft.fftfreq(iF, d=dt)
    #     nf = iF // 2 + 1  # positive frequencies
    #     freq_pos = freq[:nf]
    #
    #     # Frequency band of interest
    #     fid_min = np.searchsorted(freq_pos, self.fmin)
